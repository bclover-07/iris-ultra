...
import { Globe, Mic, Swords } from 'lucide-react';
import WorldTab from '../world/page';
import InterviewTab from '../interview/page';
import DebateTab from '../debate/page';

export default function VirtualBeyondPage() {
  const [activeTab, setActiveTab] = useState<'world' | 'interview' | 'debate'>('world');

  const tabs = [
    { id: 'world', label: 'Virtual World', icon: Globe, color: '#4CAF50' },
    { id: 'interview', label: 'Interview Hub', icon: Mic, color: '#E91E63' },
    { id: 'debate', label: 'Virtual Debate', icon: Swords, color: '#6366F1' },
  ] as const;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-white dark:bg-slate-800 p-8 rounded-3xl border-4 border-[#111] dark:border-slate-700 shadow-[8px_8px_0_#111] dark:shadow-[8px_8px_0_#1e293b]">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-12 h-12 rounded-2xl bg-indigo-100 dark:bg-indigo-950/40 flex items-center justify-center border-2 border-brutal-border dark:border-indigo-800 rotate-[-3deg]">
          ...
          <p className="font-fredoka text-gray-500 dark:text-slate-400 font-bold tracking-wide uppercase text-sm ml-1">
            Step into the Metaverse of Learning
          </p>
        </div>
          ...
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                {isActive && (
                  <motion.div
                    layoutId="virtual-beyond-tab-indicator"
