...
// GET /api/quiz/bank or /api/quizbank/:topic
router.get('/bank/:topic?', async (req, res) => {
  try {
    const topic = req.params.topic;
    let filtered = questionBank;
    if (topic && topic !== 'all' && topic !== 'General') {
      filtered = questionBank.filter(q => q.subject?.toLowerCase() === topic.toLowerCase());
      if (filtered.length === 0) filtered = questionBank;
    }
    
    // Shuffle and pick 5-10 questions
...
// POST /api/quiz/attempt (Log Camo Quizo or Standard Quiz attempt)
router.post('/attempt', async (req, res) => {
  try {
    ...
    const graded = (answers || []).map(a => {
      const q = questionBank.find(item => item.id === a.questionId || item.question === a.question);
    ...
    // Update Student Profile Quiz Mastery
